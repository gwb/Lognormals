...
large.sample = exp(rnorm(100000, u, s))
small.sample = large.sample[seq(1,5000)]
...
kl.small.th <- sum(th.density * (log(th.small.density) - log(small.sample.density$y)))
kl.large.th <- sum(th.density * (log(th.large.density) - log(large.sample.density$y)))
...
# Prints the result
print(kl.small.th)
print(kl.control.small)

print(kl.large.th)
print(kl.control.large)

print(kl.large.adjusted.th)
