...
param.file <- "params-3.csv"
base.filepath.out <- "/n/airoldifs2/lab/gbasse/lognormals/METHODS-COMPARISON/res/out"
...
test.mcmc <- function(m1, m2, sd1, sd2, method, x0=NULL, numiter=100){
  ...
  if (method=='FW'){
    source('/n/airoldifs2/lab/gbasse/lognormals/METHODS-COMPARISON/method-FW.R')
    res <- get.FW(u1, u2, s1, s2)
    n.u <- res[1]
    n.s <- res[2]
  }else if (method == 'SY'){
    source('/n/airoldifs2/lab/gbasse/lognormals/METHODS-COMPARISON/method-SY.R')
    res <- get.SY(u1, u2, s1, s2)
    n.u <- res[1]
    n.s <- res[2]
    print(n.u)
    print(n.s)
  }else if (method == 'WU'){
    source('/n/airoldifs2/lab/gbasse/lognormals/METHODS-COMPARISON/method-WU.R')
    res <- get.WU(u1, u2, s1, s2)
    n.u <- res[1]
    n.s <- res[2]
  }else if (method == 'US'){
    source('/n/airoldifs2/lab/gbasse/lognormals/METHODS-COMPARISON/method-US.R')
    res <- get.US(u1, u2, s1, s2)
    n.u <- res[1]
    n.s <- res[2]
  }
...
for (i in seq(1, row.count)){
  ...
  for (i in seq(1, number_of_simulations)){
    distance <- test.mcmc(m1, m2, sd1, sd2, method=method, x0=exp(qtile[i]), numiter=numiter)[[2]]
